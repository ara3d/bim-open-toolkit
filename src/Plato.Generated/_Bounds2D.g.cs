...
public IReadOnlyList<Point2D> Corners { [MethodImpl(AggressiveInlining)] get  => Intrinsics.MakeArray(new Point2D(this.Min.X, this.Min.Y), new Point2D(this.Max.X, this.Min.Y), new Point2D(this.Max.X, this.Max.Y), new Point2D(this.Min.X, this.Max.Y)); } 
...
public Point2D this[Integer n] { [MethodImpl(AggressiveInlining)]  get => At(n); }
        public Bounds2D Reverse { [MethodImpl(AggressiveInlining)] get  => Bounds2D.CreateFromComponents(this.Components.Reverse()); } 
