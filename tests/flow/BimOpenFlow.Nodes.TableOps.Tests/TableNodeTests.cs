...
public sealed class TableNodeTests
{
    ...
    [Test]
    public void Pack_ExposesTheFourCoreTableNodes()
        => Assert.That(TableOpsNodes.All.Select(n => n.Spec.Kind), Is.SupersetOf(new[]
        {
    ...
    {
        var result = new TableFilterNode().EvalTable(SampleTables.SampleTable(), ("expr", "Height > 2.2"));
        Assert.That(result.Rows, Has.Count.EqualTo(2));
        Assert.That(result.Cell("Height", 0), Is.EqualTo(2.5));
        Assert.That(result.Cell("Height", 1), Is.EqualTo(3.0));
        Assert.That(result.ColumnNames(), Is.EqualTo(SampleTables.SampleTable().ColumnNames()));
    }

    [Test]
    public void Filter_NullResult_ExcludesTheRow()
    {
        var result = new TableFilterNode().EvalTable(SampleTables.SampleTable(), ("expr", "Height > 0"));
    ...
    public void Filter_NonBooleanExpression_Throws()
        => Assert.That(
            () => new TableFilterNode().EvalTable(SampleTables.SampleTable(), ("expr", "Height + 1")),
    ...
    {
        Assert.That(
            () => new TableFilterNode().EvalTable(SampleTables.SampleTable(), ("expr", "Height >")),
            Throws.ArgumentException.With.Message.Contains("expr"));
        Assert.That(
            () => new TableFilterNode().EvalTable(SampleTables.SampleTable(), ("expr", "NoSuchColumn > 1")),
    ...
    {
        var result = new TableDeriveNode().EvalTable(SampleTables.SampleTable(),
    ...
    public void Derive_DuplicateColumnName_Throws()
        => Assert.That(
            () => new TableDeriveNode().EvalTable(SampleTables.SampleTable(),
                ("name", "Height"), ("expr", "1")),
    ...
    {
        var result = new TableAggregateNode().EvalTable(SampleTables.SampleTable(),
            ("groupBy", "Category"),
    ...
    public void Aggregate_RejectsBadSpecs()
    {
        ...
        Assert.That(() => node.EvalTable(SampleTables.SampleTable(),
    ...
    {
        var result = new TableSortNode().EvalTable(SampleTables.SampleTable(),
    ...
    {
        Assert.That(() => new TableSortNode().EvalTable(SampleTables.SampleTable(), ("by", "NoSuchColumn")),
            Throws.ArgumentException.With.Message.Contains("NoSuchColumn"));
        Assert.That(() => new TableSortNode().EvalTable(SampleTables.SampleTable(), ("by", "Count sideways")),
            Throws.ArgumentException);
    }
