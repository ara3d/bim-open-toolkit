...
public sealed class ShowcaseGraphTests
{
    ...
    private static EvalSnapshot EvaluateGreen(GraphDocument doc, string id, NodeRegistry registry, params string[] effectNodes)
    {
        Assert.That(doc.Validate(registry), Is.Empty, id);
        var snapshot = doc.Evaluate(registry);
        Assert.That(snapshot.Results
            .Where(r => r.Value.Status != (effectNodes.Contains(r.Key) ? NodeStatus.EffectPending : NodeStatus.Ok))
    ...
    public void IfcToVerdictsAndChart_ColoursDoors_CountsVerdicts_AndWritesTheReport()
    {
        ...
        var snapshot = EvaluateGreen(doc, "ifc-to-verdicts-and-chart", registry, "report");

        var verdicts = Table(snapshot, "answer");
        var chart = Table(snapshot, "chart");
        var coloured = Table(snapshot, "coloured");
        var pending = snapshot.Results["report"];
        var report = registry.Find("sink.report", 1)!
            .Eval(new RunContext(), pending.EffectInputs, new ParamValues(doc.Values["report"]));
        try
        {
            Assert.Multiple(() =>
            {
                ...
                Assert.That(Convert.ToInt64(((TableValue)report[0]).Table.Cell("rowCount", 0)), Is.EqualTo(14));
            });
        }
        finally
        {
