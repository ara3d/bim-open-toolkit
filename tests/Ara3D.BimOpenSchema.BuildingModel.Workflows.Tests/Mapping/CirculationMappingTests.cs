...
public sealed class CirculationMappingTests
{
    private static BuildingProjection Map(MappingFixture fixture, MappingOptions? options = null)
        => BuildingMapper.Map(fixture.Model(), options ?? MappingFixture.Options(), [CoreMapping.Domain, CirculationMapping.Domain]);
    ...
    public void Numeric_fields_require_the_revit_internal_policy_to_resolve()
    {
        ...
        var flight = declared.StairFlights.Single();
        Assert.That(flight.NominalRiserHeight, Is.TypeOf<Fact<Length>.Missing>(), "No canonical unit is established under the default policy.");
