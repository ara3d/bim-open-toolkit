...
export type LoadedBosModel = { readonly model: ModelData; readonly bindings: InstanceBinding[]; readonly bimData?: BimData };
...
export async function loadBosModel(source: LoadSource, modelRef: ModelRef, options: BosModelOptions = {}): Promise<Result<LoadedBosModel>> {
  ...
  try {
    ...
    const entityLocalIds = prepared ? await bimEntityLocalIds(prepared.bimData) : bos?.EntityLocalId;
    ...
    const addObject = (entity: number) => {
      ...
      if (prepared && entityLocalIds && entity >= entityLocalIds.length) throw new Error('BFAST entity index exceeds Entities table');
      if (!objects.has(entity)) {
        const sourceId = entityLocalIds?.[entity];
        objects.set(entity, {
    ...
    for (let entity = 0; entity < (entityLocalIds?.length ?? 0); entity++) addObject(entity);
    ...
    check();
    return { ok: true, value: { model: { ref: { ...modelRef }, coordinates: { units: 'unknown', up: 'Y', registration: 'unknown' }, objects: [...objects.values()] }, bindings, ...(prepared ? { bimData: prepared.bimData } : {}) }, diagnostics: [] };
  } catch (error) {
