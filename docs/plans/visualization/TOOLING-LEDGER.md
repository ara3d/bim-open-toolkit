...
| FA features (2026-09-08) | helpful | neutral | clean | 86 tests, plus a run of every feature through V's real session: helpful | Package-wide test runs were red from other tracks' mid-edit files; per-path runs kept the track's own result readable. |
...
- 2026-09-08, shutdown, wave 2 verdicts from the track reports above. tsc: keep (caught real defects in R, F, S2, PERF, M, S, D0). vitest: keep, best value per second everywhere; perf suites are measurements, never gates. Escape-hatch rules: keep; changed designs for the better in S, M and R and forced sentinels instead of `!`; the grep needs a parser. Typed ESLint: keep only on I/O packages; it caught one thing on demos and nothing elsewhere. Untyped ESLint: neutral in every report, kept only because it is cheap; drop if wave 3 reports the same. Screenshots: a gate for anything visual (E2E). Downstream typechecks: prove additivity, never that a reader works (R2). Standing per-fence commit grants: worked with pathspec discipline, failed once through an amend; the amend ban stands. Concurrency: seven to ten subagents ran at once without a stall once the user's connection held.
