...
- `platonic-mcp.mts` starts the platonic-ts MCP server (sibling checkout `../platonic-ts`) over the `viz/` workspace. Registered in `.mcp.json`; run by hand with `node ../platonic-ts/node_modules/tsx/dist/cli.mjs tools/platonic-mcp.mts`.
- `platonic-check.mts` runs the platonic-ts check gate (typecheck, lint, escape-hatch ratchet, V2 tests) over `viz/`. Run with `node ../platonic-ts/node_modules/tsx/dist/cli.mjs tools/platonic-check.mts`.
