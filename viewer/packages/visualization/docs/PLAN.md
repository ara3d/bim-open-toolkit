...
| Active track | Exclusive writable paths (under visualization unless stated) | Acceptance |
|---|---|---|
...
Browser evidence already obtained: full Snowdon renders; category tint and18% ghosting visibly work; plane/box clipping works; selection→move→undo restores transaction state without errors. Integration validation continues as new demos land. First submitted frame observations varied with concurrent work; no opening-time guarantee is claimed.
